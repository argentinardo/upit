...
* Assets
****/
LK.init.shape('grassMask', {width:2048, height:1640, color:0x228b22, shape:'box'})
LK.init.shape('roadMask', {width:2048, height:800, color:0x000000, shape:'box'})
LK.init.shape('skyBackground', {width:2048, height:1366, color:0x87ceeb, shape:'box'})
LK.init.shape('woodenPlank2', {width:100, height:100, color:0x612825, shape:'box'})
LK.init.shape('woodenPlank3', {width:100, height:100, color:0x208310, shape:'box'})
...
LK.init.image('Mainscreen', {width:400, height:400, id:'687da4a2bdf42f92241c550a'})
...
LK.init.image('electro-ninja', {width:100, height:100, id:'687d5d90546d3d568db4ef75'})
...
LK.init.image('lightning', {width:100, height:25, id:'687d88e8bdf42f92241c5406', orientation:1})
...
LK.init.image('ninja-ray', {width:300, height:300, id:'687d56e7c4a1a36a58c6557e'})
...
LK.init.sound('Ninjaarrowgrito', {volume:1, start:0, end:1, id:'687e25fe97be051837220948'})
LK.init.sound('aeroplane-fall')
...
LK.init.sound('lightning', {volume:0.8, start:0, end:1, id:'687d04424caaf4399f3ee1ed'})
	...
	self.startFalling = function () {
		...
		function createTrembling() {
			if (trembleCount < maxTrembles) {
				...
				// Play airplane fall sound
				LK.getSound('aeroplane-fall').play();
			}
		}
...
var CloudType2 = Container.expand(function () {
	var self = Container.call(this);
...
var ElectricalNinja = Container.expand(function () {
	var self = Container.call(this);
	var ninjaGraphics = self.attachAsset('electro-ninja', {
		anchorX: 0.5,
		anchorY: 0.5
	});
	self.active = true;
	...
	self.isJumping = true;
	self.groundY = 2532;
	self.electricalTimer = 0;
	self.frozen = false; // Track if ninja is frozen
	// Initialize scale for distance-based scaling
	self.scaleX = 0.8;
	self.scaleY = 0.8;
	// Add electrical effect every 2 seconds
	self.createElectricalEffect = function () {
		// Create lightning effect around ninja
		var lightning = new Lightning();
		lightning.x = self.x;
		lightning.y = self.y - 50;
		lightning.stretchTo(self.x, self.y + 100);
		lightnings.push(lightning);
		game.addChild(lightning);
		// Play lightning sound
		LK.getSound('lightning').play();
		// Create electrical tint effect
		tween(ninjaGraphics, {
			tint: 0x9999FF
		}, {
			duration: 200,
	...
	self.update = function () {
		...
		self.x += self.velocityX;
		self.y += self.velocityY;
		// Scale ninja based on distance to player ninja
		if (ninja) {
		...
		// Handle electrical effects
		self.electricalTimer++;
		if (self.electricalTimer >= 120) {
			// Every 2 seconds
			self.createElectricalEffect();
			self.electricalTimer = 0;
		}
		// Check if ninja hits ground (only if not frozen)
		if (!self.frozen && self.y >= self.groundY) {
			self.y = self.groundY;
			self.velocityY = 0;
			self.isJumping = false;
		}
		// Remove if off screen
		if (self.x < -200 || self.x > 2248 || self.y > 3300) {
			self.active = false;
		}
	};
	return self;
...
var Enemy = Container.expand(function () {
	...
	// Initialize scale for distance-based scaling
	self.scaleX = 0.5; // Start larger when spawned far away
	self.scaleY = 0.5;
	self.update = function () {
		if (!self.active || game.paused) {
			return;
		...
		if (ninja) {
			...
			var minScale = 0.5; // Minimum scale when far away
			var maxScale = 2.0; // Maximum scale when very close
			var normalizedDistance = Math.min(distance / maxDistance, 1);
			var targetScale = maxScale - normalizedDistance * (maxScale - minScale);
		...
		// Check if enemy hits ground (only if not frozen)
		if (!self.frozen && self.y >= self.groundY) {
			self.y = self.groundY;
			self.velocityY = 0;
			self.isJumping = false;
...
var EnemyArcher = Container.expand(function () {
	...
	self.shootTimer = 0;
	self.shootInterval = 15; // 0.25 seconds at 60fps - shoot arrows much more frequently
	self.detectionRange = 5000; // Even larger range to detect ninja from very far away
	...
	self.update = function () {
		...
		if (self.tree && self.tree.active) {
			...
			// Check if ninja is within range and at similar height for shooting
			if (ninja) {
				var dx = ninja.x - self.x;
				var dy = ninja.y - self.y;
				var distance = Math.sqrt(dx * dx + dy * dy);
				// Check if ninja is at approximately the same height (within 400px vertically) - increased range
				var heightDifference = Math.abs(dy);
				var isSameHeight = heightDifference < 1000;
				...
				if (isSameHeight && isInRange) {
					// Ninja is at same height and within shooting range - increment shoot timer
					self.shootTimer++;
					...
					// Ninja is too far - reset timer to prevent shooting when not in range
					self.shootTimer = 0;
				}
			}
		} else {
...
var Lightning = Container.expand(function () {
	...
	self.active = true;
	self.lifeTimer = 0; // Timer to track lightning lifetime
	self.maxLifetime = 120; // Lightning lasts 2 seconds (120 frames)
	self.update = function () {
		if (!self.active || game.paused) {
			return;
		}
		self.lifeTimer++;
		// Continuously update lightning to stretch from cloud to ninja head level
		self.stretchTo(ninja ? ninja.x : 1024, ninja ? ninja.y - 200 : 2532);
		if (self.lifeTimer >= self.maxLifetime) {
			self.active = false;
		}
	};
	// Method to stretch lightning from cloud bottom to ninja head
	self.stretchTo = function (targetX, targetY) {
		if (!ninja) {
			return;
		} // Safety check
		// Calculate cloud bottom position (most bottom part of the cloud)
		var cloud = self.parent; // Lightning should be attached to a cloud
		var cloudBottomY = cloud ? cloud.y + cloud.scaleY * 40 : self.y; // Bottom edge of cloud based on its scale
		// Calculate ninja head position (top part of ninja)
		var ninjaHeadY = ninja.y - (ninja.scaleY ? ninja.scaleY * 300 : 300); // Higher up on ninja body
		// Calculate distance from cloud bottom to ninja head
		var dx = ninja.x - self.x;
		var dy = ninjaHeadY - cloudBottomY;
		var distance = Math.sqrt(dx * dx + dy * dy);
		// Calculate rotation angle to point from cloud bottom toward ninja head
		var angle = Math.atan2(dy, dx) - Math.PI / 2; // Subtract PI/2 since lightning asset points up by default
		// Scale the lightning properly - stretch it to cover the distance from cloud bottom to ninja head
		lightningGraphics.scaleY = distance / 50; // Scale Y to stretch along the length
		lightningGraphics.scaleX = 2; // Make it visible but not too thick
		lightningGraphics.rotation = angle; // Point toward ninja head
		// Position lightning to start at the bottom of the cloud, raised by 180px
		var cloudBottomOffset = cloud && cloud.scaleY ? cloud.scaleY * 40 : 40;
		lightningGraphics.y = cloudBottomOffset - 180; // Start from bottom edge of cloud, raised 180px higher
	};
	return self;
});
...
var Ninja = Container.expand(function () {
	...
	self.jump = function () {
		if (self.jumpCount < self.maxJumps && !ninjaParalyzed) {
			...
			// Change sprite to ninja-ray during jump
			ninjaGraphics.destroy();
			ninjaGraphics = self.attachAsset('ninja-ray', {
				anchorX: 0.5,
				anchorY: 0.5
			});
			// Add 3 rotations during jump (3 full rotations = 6π radians)
			tween(ninjaGraphics, {
				rotation: Math.PI * 6
			}, {
				duration: 800,
			...
			// Determine jump height - second jump goes higher than first
...
var WoodenPlank2 = Container.expand(function () {
	var self = Container.call(this);
	var plankGraphics = self.attachAsset('woodenPlank2', {
		anchorX: 0.5,
		anchorY: 0.5
	});
	self.speed = 3;
	...
	self.update = function () {
		...
		self.z = self.z || 1000;
		var distanceFactor = Math.max(0.1, 1 - self.z / 1000);
		var currentSpeed = self.speed * (0.3 + distanceFactor * 2);
		self.z -= currentSpeed;
		var scale = Math.max(0.1, 1 - self.z / 1000);
		var horizonY = 1503;
		var groundY = 2532;
		...
		self.x = self.baseX;
		var plankScale = scale * 2;
		self.scaleX = plankScale;
		var verticalScale = scale * 1.665;
		self.scaleY = verticalScale;
...
var WoodenPlank3 = Container.expand(function () {
	var self = Container.call(this);
	var plankGraphics = self.attachAsset('woodenPlank3', {
		anchorX: 0.5,
		anchorY: 0.5
	});
	self.speed = 3;
...
var WoodenPlank4 = Container.expand(function () {
	var self = Container.call(this);
	var plankGraphics = self.attachAsset('woodenPlank4', {
		anchorX: 0.5,
		anchorY: 0.5
	});
	self.speed = 3;
...
var electricalNinjaSpawnTimer = 0;
var enemyArchers = [];
var arrows = [];
...
// Font will use system fallbacks for retro pixelated typography
// Press Start 2P font family defined in scoreTxt creation with fallbacks
// Create score display with improved styling
var scoreTxt = new Text2('Score: 0 | Enemies: 0', {
	size: 80,
	fill: 0xFFD700,
	// Gold color for better visibility
	font: "'Press Start 2P', 'Courier New', monospace" // Retro pixelated font family
});
...
// Add drop shadow effect with slight offset
...
function updateScore() {
	scoreTxt.setText('Score: ' + LK.getScore() + ' | Enemies: ' + enemiesKilled);
}
...
// Spawn electrical ninja from tree crown - DISABLED
function spawnElectricalNinja() {
	// ElectricalNinja enemy disabled to prevent electrocutado asset enemy from appearing
	return;
}
...
function spawnObstacle() {
	...
	for (var t = 0; t < toris.length; t++) {
		var tori = toris[t];
		if (tori.active && tori.z && tori.z > 800 && tori.z < 1200) {
	...
	if (!canSpawnObstacle) {
		return; // Skip obstacle spawning to avoid collision with tori
	}
...
function spawnTori() {
	...
	for (var o = 0; o < obstacles.length; o++) {
		var obstacle = obstacles[o];
		if (obstacle.active && obstacle.z && obstacle.z > 800 && obstacle.z < 1200) {
	...
	if (!canSpawnTori) {
		return; // Skip tori spawning to avoid collision with obstacle
	}
	var tori = new Tori();
...
game.update = function () {
	...
	var _loop3 = function _loop3() {
			...
			for (j = enemies.length - 1; j >= 0; j--) {
				...
				if (enemy.active && iceStar.intersects(enemy)) {
					...
					// Change enemy to freeze version instead of destroying
					enemy.frozen = true; // Mark enemy as frozen
					var enemyGraphics = enemy.children[0];
					enemyGraphics.destroy();
					var freezeGraphics = enemy.attachAsset('freeze-enemy', {
						anchorX: 0.5,
						anchorY: 0.5
					});
					// Apply ice blue tint and create freeze effect
					tween(freezeGraphics, {
						tint: 0x87CEEB
					}, {
						duration: 300,
						easing: tween.easeOut
			...
			for (j = enemies2.length - 1; j >= 0; j--) {
				...
				if (enemy2.active && iceStar.intersects(enemy2)) {
					...
					updateScore();
					LK.getSound('Congelado').play();
					// Change enemy2 to freeze version instead of destroying
					enemy2.frozen = true; // Mark enemy2 as frozen
					var enemy2Graphics = enemy2.children[0];
					enemy2Graphics.destroy();
					var freeze2Graphics = enemy2.attachAsset('freeze-enemy2', {
						anchorX: 0.5,
						anchorY: 0.5
					});
					// Apply ice blue tint and create freeze effect
					tween(freeze2Graphics, {
						tint: 0x87CEEB
					}, {
						duration: 300,
						easing: tween.easeOut
			...
			for (j = enemies3.length - 1; j >= 0; j--) {
				...
				if (enemy3.active && iceStar.intersects(enemy3)) {
					...
					updateScore();
					LK.getSound('Congelado').play();
					// Change enemy3 to freeze version instead of destroying
					enemy3.frozen = true; // Mark enemy3 as frozen
					var enemy3Graphics = enemy3.children[0];
					enemy3Graphics.destroy();
					var freeze3Graphics = enemy3.attachAsset('freeze-enemy3', {
						anchorX: 0.5,
						anchorY: 0.5
					});
					// Apply ice blue tint and create freeze effect
					tween(freeze3Graphics, {
						tint: 0x87CEEB
					}, {
						duration: 300,
						easing: tween.easeOut
			...
			for (j = electricalNinjas.length - 1; j >= 0; j--) {
				electricalNinja = electricalNinjas[j];
				if (electricalNinja.active && iceStar.intersects(electricalNinja)) {
					enemiesKilled++;
					LK.setScore(LK.getScore() + 30);
					updateScore();
					LK.getSound('Congelado').play();
					// Change electrical ninja to freeze version instead of destroying
					electricalNinja.frozen = true; // Mark electrical ninja as frozen
					var electricalGraphics = electricalNinja.children[0];
					electricalGraphics.destroy();
					var freezeElectricalGraphics = electricalNinja.attachAsset('frezze-airplane', {
						anchorX: 0.5,
						anchorY: 0.5
					});
					// Apply ice blue tint and create freeze effect
					tween(freezeElectricalGraphics, {
						tint: 0x87CEEB
					}, {
						duration: 300,
						easing: tween.easeOut
			...
			for (j = enemyArchers.length - 1; j >= 0; j--) {
				...
				if (archer.active && iceStar.intersects(archer)) {
					...
					updateScore();
					LK.getSound('Congelado').play();
					// Change archer to freeze version instead of destroying
					archer.frozen = true; // Mark archer as frozen
					var archerGraphics = archer.children[0];
					archerGraphics.destroy();
					var freezeArcherGraphics = archer.attachAsset('freeze-archer-ninja', {
						anchorX: 0.5,
						anchorY: 1.0
					});
					// Apply ice blue tint and create freeze effect
					tween(freezeArcherGraphics, {
						tint: 0x87CEEB
					}, {
						duration: 300,
						easing: tween.easeOut
			...
			for (var cl = clouds.length - 1; cl >= 0; cl--) {
				var cloud = clouds[cl];
				if (cloud.active && cloud.constructor === CloudType2 && iceStar.intersects(cloud)) {
					// Play electric cloud hit sound
					LK.getSound('electric-cloud-hit').play();
					// Change cloud to regular cloud asset (disable electric properties)
					...
					var originalTint = cloudGraphics.tint;
					// Destroy electric cloud graphics
					cloudGraphics.destroy();
					// Replace with regular cloud graphics
					var newCloudGraphics = cloud.attachAsset('cloud', {
						anchorX: 0.5,
						anchorY: 0.5,
					...
					newCloudGraphics.tint = originalTint;
					...
					// Add freeze tint effect to show it was hit by ice
					tween(newCloudGraphics, {
						tint: 0x87CEEB
					}, {
						duration: 300,
						easing: tween.easeOut
	...
	for (var i = woodenStars.length - 1; i >= 0; i--) {
		...
		for (var j = enemies.length - 1; j >= 0; j--) {
			...
			if (enemy.active && woodenStar.intersects(enemy)) {
				...
				// Remove both wooden star and enemy
				woodenStar.active = false;
				enemy.active = false;
				woodenStar.destroy();
				enemy.destroy();
		...
		// Check collision with electrical ninjas
		for (var j = electricalNinjas.length - 1; j >= 0; j--) {
			var electricalNinja = electricalNinjas[j];
			if (electricalNinja.active && woodenStar.intersects(electricalNinja)) {
				// Hit electrical ninja
				enemiesKilled++;
				LK.setScore(LK.getScore() + 30);
				updateScore();
				LK.getSound('lightning').play();
				// Remove both wooden star and ninja
				woodenStar.active = false;
				electricalNinja.active = false;
				woodenStar.destroy();
				electricalNinja.destroy();
				woodenStars.splice(i, 1);
				electricalNinjas.splice(j, 1);
				break;
			}
		}
		...
		for (var j = enemyArchers.length - 1; j >= 0; j--) {
			...
			if (archer.active && woodenStar.intersects(archer)) {
				...
				// Remove both wooden star and archer
				woodenStar.active = false;
				archer.active = false;
				woodenStar.destroy();
				archer.destroy();
				woodenStars.splice(i, 1);
				enemyArchers.splice(j, 1);
				break;
			}
		}
	...
	for (var i = ninjaStars.length - 1; i >= 0; i--) {
		...
		var _loop2 = function _loop2() {
				cloud = clouds[c];
				if (cloud.active && star.intersects(cloud) && cloud.constructor === CloudType2) {
					...
					if (cloud.z <= 300) {
						...
						// Spawn lightning that stretches from cloud to center bottom of screen
						lightning = new Lightning();
						lightning.x = cloud.x;
						lightning.y = cloud.y;
						// Stretch lightning from cloud to center bottom of screen
						lightning.stretchTo(1024, 2732);
						lightnings.push(lightning);
						game.addChild(lightning);
						// Play lightning sound
						LK.getSound('lightning').play();
		...
		for (var j = electricalNinjas.length - 1; j >= 0; j--) {
			var electricalNinja = electricalNinjas[j];
			if (electricalNinja.active && star.intersects(electricalNinja)) {
				// Hit electrical ninja
				enemiesKilled++;
				LK.setScore(LK.getScore() + 30); // Most points for electrical ninja
				updateScore();
				LK.getSound('lightning').play(); // Lightning sound for hit
				// Remove both star and enemy
				star.active = false;
				electricalNinja.active = false;
				star.destroy();
	...
	for (var i = enemies.length - 1; i >= 0; i--) {
		...
		// Check collision with ninja (only in lower part of game when enemy is close and not frozen)
		if (enemy.y > 2200 && !enemy.frozen && ninja.intersects(enemy)) {
	...
	for (var i = enemies2.length - 1; i >= 0; i--) {
		...
		// Check collision with ninja (only in lower part of game when enemy is close and not frozen)
		if (enemy2.y > 2200 && !enemy2.frozen && ninja.intersects(enemy2)) {
			LK.getSound('ninja-ouch').play();
			// Turn screen red and stop ninja walking
			tween(game, {
				tint: 0xff0000
	...
	for (var i = enemies3.length - 1; i >= 0; i--) {
		...
		// Check collision with ninja (only in lower part of game when enemy is close and not frozen)
		if (enemy3.y > 2200 && !enemy3.frozen && ninja.intersects(enemy3)) {
			LK.getSound('ninja-ouch').play();
			// Turn screen red and stop ninja walking
			tween(game, {
				tint: 0xff0000
	...
	// Update electrical ninjas
	for (var i = electricalNinjas.length - 1; i >= 0; i--) {
		var electricalNinja = electricalNinjas[i];
		electricalNinja.update();
		if (!electricalNinja.active) {
			electricalNinja.destroy();
			electricalNinjas.splice(i, 1);
			continue;
		}
		// Check collision with ninja (only in lower part of game when ninja is close and not frozen)
		if (electricalNinja.y > 2200 && !electricalNinja.frozen && ninja.intersects(electricalNinja)) {
			LK.getSound('ninja-ouch').play();
			// Turn screen red and stop ninja walking
			tween(game, {
				tint: 0xff0000
	...
	if (!mainScreenActive) {
		...
		// Spawn electrical ninjas - DISABLED to prevent electrocutado asset enemy
		electricalNinjaSpawnTimer++;
		// ElectricalNinja spawning disabled
		// Spawn enemy archers on trees
		if (LK.ticks % 900 === 0) {
			// Every 15 seconds
			spawnEnemyArcher();
		}
	...
	if (ninjaParalyzed && ninjaParalyzeTimer > 0) {
		...
		if (ninjaParalyzeTimer <= 0) {
			// Recover from paralysis - only change graphics once
			ninjaParalyzed = false;
			// Store current state before cleanup
			var wasJumping = ninja.isJumping;
			var currentY = ninja.y;
			var currentFlipDirection = ninja.flipDirection;
			// Remove all existing graphics completely
			while (ninja.children.length > 0) {
				ninja.children[0].destroy();
			}
			// Check if ninja was jumping when paralyzed and resume jump if in air
			if (wasJumping && currentY < ninja.groundY) {
				// Add ninja-ray graphics for jumping state
				var ninjaRayGraphics = ninja.attachAsset('ninja-ray', {
					anchorX: 0.5,
					anchorY: 0.5
				});
				// Set flip direction for running animation
				ninjaRayGraphics.scaleX = currentFlipDirection;
				// Reset ninja rotation to prevent stuck rotation from electrocution
				ninjaRayGraphics.rotation = 0;
				// Continue fall to ground
				tween(ninja, {
					y: ninja.groundY
				}, {
					duration: Math.max(200, (ninja.groundY - currentY) * 0.5),
					easing: tween.easeIn,
					onFinish: function onFinish() {
						ninja.isJumping = false;
						ninja.jumpVelocity = 0;
						// Remove jumping graphics and add normal ninja sprite
						while (ninja.children.length > 0) {
							ninja.children[0].destroy();
						}
						...
						// Set flip direction for running animation
						normalNinjaGraphics.scaleX = currentFlipDirection;
						// Reset rotation to ensure normal state
						normalNinjaGraphics.rotation = 0;
					}
				});
			} else {
				// Add normal ninja graphics for ground state - ninja is on ground and should walk normally
				var normalNinjaGraphics = ninja.attachAsset('ninja', {
					anchorX: 0.5,
					anchorY: 1.0
				});
				...
				// Reset rotation to ensure normal state
				normalNinjaGraphics.rotation = 0;
				// Reset ninja properties to ensure proper walking state
				ninja.isJumping = false;
				ninja.jumpVelocity = 0;
			}
		}
